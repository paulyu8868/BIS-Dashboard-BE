...
public interface RouteRepository extends JpaRepository<M_OP_ROUTE_POINT, RoutePointId> {
    ...
    // 모든 노드 조회
    @Query("SELECT n FROM M_OP_ROUTE_POINT r JOIN M_TP_NODE n ON r.pointId = n.nodeId " +
            "WHERE r.routeId = :routeId ORDER BY r.pointSqno")
    List<M_TP_NODE> findAllNodesByRouteId(@Param("routeId") String routeId);
